...
package cloudproy.gennumcloud_0_1;
...
/**
 * Job: genNumCloud Purpose: genNumCloud<br>
 * Description: genNumCloud <br>
 * @author Sanchez, Lilian
 * @version 8.0.1.20250218_0945-patch
 * @status 
 */
public class genNumCloud implements TalendJob {
	static {System.setProperty("TalendJob.log", "genNumCloud.log");}

	

	
	private static org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager.getLogger(genNumCloud.class);
...
protected static void logIgnoredError(String message, Throwable cause) {
	...
	private final String jobName = "genNumCloud";
	private final String projectName = "CLOUDPROY";
...
private final JobStructureCatcherUtils talendJobLog = new JobStructureCatcherUtils(jobName, "_GSfPEEIzEfC2yfUAj20mPQ", "0.1");
		...
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(genNumCloud.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
    final static byte[] commonByteArrayLock_CLOUDPROY_genNumCloud = new byte[0];
    static byte[] commonByteArray_CLOUDPROY_genNumCloud = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_CLOUDPROY_genNumCloud.length) {
				if(length < 1024 && commonByteArray_CLOUDPROY_genNumCloud.length == 0) {
   					commonByteArray_CLOUDPROY_genNumCloud = new byte[1024];
				} else {
   					commonByteArray_CLOUDPROY_genNumCloud = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_CLOUDPROY_genNumCloud, 0, length);
			strReturn = new String(commonByteArray_CLOUDPROY_genNumCloud, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_CLOUDPROY_genNumCloud, 0, length);
			strReturn = new String(commonByteArray_CLOUDPROY_genNumCloud, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_CLOUDPROY_genNumCloud) {
 ...
 */
	...
	s(currentComponent="tLogRow_1");
			...
			if(runStat.update(execStat,enableLogStash,iterateId,1,1
					...
					,"row1","tRowGenerator_1","tRowGenerator_1","tRowGenerator","tLogRow_1","tLogRow_1","tLogRow"
...
///////////////////////    			
			 		...
			 		if(runStat.updateStatAndLog(execStat,enableLogStash,resourceMap,iterateId,"row1",2,0,
			 			"tRowGenerator_1","tRowGenerator_1","tRowGenerator","tLogRow_1","tLogRow_1","tLogRow","output")) {
						talendJobLogProcess(globalMap);
					}
 ...
 */
    ...
    public static void main(String[] args){
        final genNumCloud genNumCloudClass = new genNumCloud();

        int exitCode = genNumCloudClass.runJobInTOS(args);
	        if(exitCode==0){
		        log.info("TalendJob: 'genNumCloud' - Done.");
	        }
    ...
    private void readJobInfo(java.io.File jobInfoFile){
		...
		log.info(String.format("Project name: %s\tJob name: %s\tGIT Commit ID: %s\tTalend Version: %s",
				projectName,jobName,jobInfo.getProperty("gitCommitId"), "8.0.1.20250218_0945-patch"));
    ...
    public int runJobInTOS(String[] args) {
        ...
        enableLogStash = "true".equalsIgnoreCase(System.getProperty("audit.enabled"));
	        ...
	        getjobInfo();
			log.info("TalendJob: 'genNumCloud' - Start.");
                ...
                org.slf4j.MDC.put("_jobRepositoryId","_GSfPEEIzEfC2yfUAj20mPQ");
                org.slf4j.MDC.put("_compiledAtTimestamp","2025-06-05T17:50:47.921172113Z");
        ...
        try {
            ...
            java.io.InputStream inContext = genNumCloud.class.getClassLoader().getResourceAsStream("cloudproy/gennumcloud_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = genNumCloud.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
        ...
        resumeUtil.addLog("JOB_STARTED", "JOB:" + jobName, parent_part_launcher, Thread.currentThread().getId() + "", "","","","",resumeUtil.convertToJsonText(context,ContextProperties.class,parametersToEncrypt));
            ...
            log.info("TalendJob: 'genNumCloud' - Started.");
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : genNumCloud");
        }
		if(enableLogStash) {
...
if (execStat) {
    ...
    resumeUtil.flush();
        ...
        log.info("TalendJob: 'genNumCloud' - Finished - status: " + status + " returnCode: " + returnCode );
...
/************************************************************************************************
 *     64234 characters generated by Talend Cloud Data Integration 
 *     on the 5 de junio de 2025, 11:50:47 CST
 ************************************************************************************************/